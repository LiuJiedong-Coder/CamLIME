...
if __name__ == '__main__':
    ...
    target_layers_alex = [model_alex.features[12]]   #alexnet
    ...
    for model, target_layers in model_and_target_layers:
        ...
        model_pred.append(pred_id)
        title_figs['Original'] = img_pil
        title_figs['LIME'] = img_boundry_org

        # 将class_act_mode中每个grayscale_cam添加到title_figs中
        # for idx, img in enumerate(class_act_mode):
        #     title_figs[f'Activation_{idx}'] = img

        # 将img_boundry_act中每个img_boundry_our添加到title_figs中
        for idx, img in enumerate(img_boundry_act):
            title_figs[f'Our_{idx}'] = img
        model_figs[model.__class__.__name__] = title_figs
        print()

    # 绘图
    fig, axes = plt.subplots(len(model_figs), 2+len(methods), figsize=(1*(2+len(methods)), 1 * len(model_figs)))      #只画解释图
    #fig, axes = plt.subplots(len(model_figs), 2+len(methods)*2, figsize=(1*(2+len(methods)*2), 1 * len(model_figs)))  #画出激活和解释图
    ...
    for ix, (model, images) in enumerate(model_figs.items()):
        for jx, (title, fig) in enumerate(images.items()):
            ...
            # if jx == 0:
            #     axes[ix, jx].set_title(f'{title}:{997}-{model}:{model_pred[ix]}', fontsize=8)
            # else:
